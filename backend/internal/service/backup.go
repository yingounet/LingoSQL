...
func (s *MaintenanceService) runMySQLBackup(dbConfig *models.DbConfig, password, backupDir, database string, tables []string, compress, schemaOnly bool, maxFileSizeMB int, taskID int, taskService *TaskService) ([]struct {
	...
	// mysqldump -h HOST -P PORT -u USER [--no-data] DATABASE [tables...]
	// 密码通过 MYSQL_PWD 环境变量传递，避免出现在进程列表中
	args := []string{
		"-h", dbConfig.Host,
	...
	if dbConfig.Options != nil && dbConfig.Options.Charset != "" {
		args = append(args, "--default-character-set="+dbConfig.Options.Charset)
	}
	...
	if err != nil {
		return nil, 0, fmt.Errorf("创建 mysqldump 输出管道失败: %w", err)
	}
	if err := cmd.Start(); err != nil {
		return nil, 0, fmt.Errorf("启动 mysqldump 失败（请确保已安装 mysql-client）: %w", err)
	}
	...
	for {
		...
		if len(line) > 0 {
			if currentFile == nil {
				currentName = fmt.Sprintf("part_%03d%s", part, ext)
				currentFile, err = os.Create(filepath.Join(backupDir, currentName))
				if err != nil {
					cmd.Process.Kill()
					return nil, 0, fmt.Errorf("创建备份文件失败: %w", err)
				}
				if compress {
					gzWriter = gzip.NewWriter(currentFile)
				}
			}

			var w io.Writer = currentFile
			if compress {
				w = gzWriter
			}
			...
			if maxBytes > 0 && currentSize >= maxBytes {
				...
				currentFile, gzWriter, currentSize, part = nil, nil, 0, part+1
			}
		}
		if err == io.EOF {
			break
	...
	if err := cmd.Wait(); err != nil {
		return nil, 0, fmt.Errorf("mysqldump 执行失败: %w", err)
	}
...
func (s *MaintenanceService) runPostgreSQLBackup(dbConfig *models.DbConfig, password, backupDir, database string, tables []string, compress, schemaOnly bool, maxFileSizeMB int, taskID int, taskService *TaskService) ([]struct {
	...
	// pg_dump -h HOST -p PORT -U USER -d DATABASE [--schema-only] [-t table]...
	args := []string{
		"-h", dbConfig.Host,
		"-p", fmt.Sprintf("%d", dbConfig.Port),
		"-U", dbConfig.Username,
	...
	stdout, err := cmd.StdoutPipe()
	if err != nil {
		return nil, 0, fmt.Errorf("创建 pg_dump 输出管道失败: %w", err)
	}
	if err := cmd.Start(); err != nil {
		return nil, 0, fmt.Errorf("启动 pg_dump 失败（请确保已安装 postgresql-client）: %w", err)
	}
	...
	if err := cmd.Wait(); err != nil {
		return nil, 0, fmt.Errorf("pg_dump 执行失败: %w", err)
	}
